...
def get_logn_scale(tensor, n_head, max_pos):
    ## 在计算scale
    ## n_head * max_pos
    train_seq_len = 4096
    # 动态缩放，训练长度内不进行缩放
    if max_pos <= train_seq_len:
        scale = torch.Tensor(1).unsqueeze(0).expand(n_head, max_pos)
    else:
        position_point = torch.arange(max_pos) + 1
        position_point = position_point.unsqueeze(0).expand(n_head, -1)
        lambda_ = torch.Tensor(_get_interleave_power_of_2(n_head))
        lambda_ = lambda_.unsqueeze(1).expand(-1, max_pos)
        ## 计算scaling: n_head * max_pos
        scale = torch.log((1 - torch.exp(-position_point * lambda_)) / (1 - torch.exp(-lambda_)) - \
                          (position_point * lambda_ * torch.exp(-lambda_ * position_point)) / (
                                  1 - torch.exp(-lambda_ * position_point))
                          )
    return scale
...
class BaichuanAttention(torch.nn.Module):
    ...
    def forward(
            ...
            logn_scale: Optional[Tuple[torch.Tensor]] = None
        ...
        if xops is not None and self.training:
            ...
            if q_len ==1:
                ...
                logn_scale = logn_scale[:, -1:]
            query_states = logn_scale.unsqueeze(0).unsqueeze(-1).expand(bsz, -1, -1, self.head_dim) \
                           * query_states
    ...
    def get_alibi_mask(self, tensor, seq_length_with_past):
        if self.training:
            ...
            if self.first_run:
                ...
                self.register_buffer(
                    'logn_scale',
                    get_logn_scale(tensor, self.n_head, self.max_cache_pos).to(
                        tensor
                    ),
                    persistent=False,
                )
            ...
            print(self.max_cache_pos)
            mask = self.future_mask[
                   : self.n_head, :seq_length_with_past, :seq_length_with_past
                   ]
            scale = self.logn_scale[
                    : self.n_head, :seq_length_with_past
                    ]
        return mask, scale
    ...
    def forward(
        ...
        if self.training:
            if (
                ...
                self.alibi_mask, _ = self.get_alibi_mask(
                    inputs_embeds, seq_length_with_past
                )
            alibi_mask = self.alibi_mask
        else:
            alibi_mask, logn_scale = self.get_alibi_mask(inputs_embeds, seq_length_with_past)
