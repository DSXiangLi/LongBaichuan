...
def build_dynamically_alibi_tensor(n, max_pos) -> torch.Tensor:
    """
    Input
        n: num_head
        max_pos
    Calculation
        ...
        a = if max_pos<train_seq :1 else: seq/train_seq
        b = a^(1/(H-1))
        m_h =b / {2^(8/H) * b}^h
    """
    a0 = 1.0
    ...
    # if current_pos < train_length: 1
    a = a.masked_fill(a < 1.0, 1.0)
    scale = a ** (1.0 / (n - 1))

    def _get_interleave_power_of_2(n, scale):
        base = (2 ** (-2 ** -(math.log2(n) - 3)))
        base /= scale
        slopes = [scale * base * base ** i for i in range(n)]
        return slopes

    if math.log2(n).is_integer():
        return _get_interleave_power_of_2(n, scale)
    else:
        closest_power_of_2 = 2 ** math.floor(math.log2(n))
        return (
                _get_interleave_power_of_2(closest_power_of_2, scale)
